...
class WaterWatchMobileController extends GetxController{
  ...
  gotoOTP() async {
    ...
    try {
      ...
      var params = jsonEncode({ "mobile_number": mobile.value.text });
      ...
      if (response.statusCode != 200) {
        ...
        decode['message'] != 'OTP sent successfully'
            ? Get.snackbar("warning".tr, "${decode['message']}", backgroundColor: Colors.red, colorText: Colors.white):
        Get.to(WaterWatchOtp(), arguments: {'mobile': mobile.value.text, 'otp': decode['otp']}, transition: Transition.leftToRight);
      }
    } catch (e) {
