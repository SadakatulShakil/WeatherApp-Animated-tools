...
class WaterWatchOtpController extends GetxController {
  ...
  Future<void> loginClick() async {
    ...
    try {
      var params = jsonEncode({
        ...
        "lat": userService.lat ?? 0.0,
        "long": userService.lon ?? 0.0,
        "fcm_token": userService.fcmToken ?? "",
        "device_info": {
  ...
  Future<void> resendOTP() async {
    try {
      ...
      final response = await http.post(Uri.parse(ApiURL.VERIFY_OTP), body: params, headers: {"Content-Type": "application/json"});
      final decode = jsonDecode(response.body);
